...
namespace BrikBotCore.Attributes
{
	public static class PropertyMapHelper
	{
		...
		private static void ParsePrimitive(PropertyInfo prop, object entity, object value)
		{
			...
			{
				double number;
				var isValid = double.TryParse(value.ToString(), out number);
				if (isValid) prop.SetValue(entity, double.Parse(value.ToString()), null);
			}
			else if (prop.PropertyType == typeof(DateTime) || prop.PropertyType == typeof(DateTime?))
			{
				DateTime date;
				var isValid = DateTime.TryParse(value.ToString(), out date);
				if (isValid)
				{
				...
				else
				{
					...
					if (isValid) prop.SetValue(entity, date, null);
				}
			}
			else if (prop.PropertyType == typeof(Guid))
			{
				Guid guid;
				var isValid = Guid.TryParse(value.ToString(), out guid);
				if (isValid)
				{
				...
				else
				{
					...
					if (isValid) prop.SetValue(entity, guid, null);
				}
			}
		}
